...
export class TaskCore extends GYI {
    ...
    async makeTask(mode: Function) {
        ...
        await Promise.all(map(this.store[name], async config => {
            ...
            if (series) {
                const action = seriesFun(...series.concat([exec]));
                if (option.option && option.option.description) {
                    action.description = option.option.description;
                }
                return await gulp.task(key, action);
            } else if (parallel) {
                const action = parallelFun(...parallel.concat([exec]));
                if (option.option && option.option.description) {
                    action.description = option.option.description;
                }
                return await gulp.task(key, action);
            ...
            if (option.option && option.option.description) {
                (exec as any).description = option.option.description;
            }
